...
import { SysAssistRailLockup, SysAssistSymbol } from './BrandLogo';
import { LedIndicator } from './SkeuoComponents';
import { soundFx } from '../services/audioService';

export type ActiveTab = 'inventory' | 'stock_tracker' | 'jira' | 'audit_trail';
  ...
  const railContent = (
    <div className="h-full flex flex-col justify-between text-[#B3B6B9] font-sans">
      ...
      <div>
          ...
          <div className="px-3 pt-3 pb-1">
            <div className="px-2.5 py-1.5 rounded bg-[#1C1F23] border border-[#2B3036] flex items-center justify-between text-[11px]">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[#9A9DA1] font-medium">Telemetry Node</span>
              </div>
        ...
        <div className="p-2 pt-3 border-t border-[#23272B] mt-2 space-y-1.5">
          ...
          >
            ...
            {!isCollapsed && <span>Barcode Scanner</span>}
          </button>
      ...
      <div className="p-2 border-t border-[#23272B]">
        {/* Operator Info */}
        {!isCollapsed ? (
          <div className="p-2 rounded bg-[#181B1E] border border-[#272B30] flex items-center gap-2.5 mb-2">
          ...
          <div className="flex justify-center mb-2" title={`Operator: ${currentUser}`}>
        ...
        {/* Controls Row: Sound Toggle & System indicator */}
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between px-1'}`}>
          <button
            onClick={onToggleSound}
            className="flex items-center gap-1.5 p-1.5 rounded text-[#7B7F83] hover:text-[#FAF9F5] hover:bg-[#202428] text-xs transition-colors cursor-pointer"
            title={soundEnabled ? 'Tactile Audio: Enabled (Click to Mute)' : 'Tactile Audio: Muted'}
            aria-label={soundEnabled ? 'Mute audio' : 'Enable audio'}
          >
            {soundEnabled ? (
              <Volume2 className="w-3.5 h-3.5 text-[#C66A2B]" />
            ) : (
              <VolumeX className="w-3.5 h-3.5 text-[#65686B]" />
            )}
            {!isCollapsed && (
          ...
          {!isCollapsed && (
            <span className="text-[10px] font-mono text-[#585B5E]">
              REV. 2026.4
            </span>
          )}
        </div>
